...
MCP_SERVER_BASE_URL = os.getenv("MCP_SERVER_URL", "http://localhost:8080")
...
class MCPClient:
    """Client for communicating with remote MCP server"""

    def __init__(self, base_url: str = MCP_SERVER_BASE_URL, timeout: int = MCP_SERVER_TIMEOUT):
    ...
    async def call_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """Make a remote call to MCP server tool"""
        try:
    ...
    try:
        ...
        # Handle element selection from available options
            ...
            if selected_element_name:
                # This will need to be made async when called from async context
                # For now, we'll handle this in the orchestrator
                return {
                    **state,
                    ...
                    "requires_human_approval": False,
                    "human_feedback": None,
                    "current_step": "element_search_and_setup"
                }
...
async def element_search_and_setup_agent(state: LineageState):
    """Helper agent to handle async element search after human approval."""
    logger.info("---EXECUTING: Element Search and Setup---")

    element_name = state.get("element_name")
    if not element_name:
        return {**state, "error_message": "No element name provided", "current_step": "error"}

    try:
        # Search for the element in database via MCP
        element_search_result = await find_element_by_name(element_name)
        if element_search_result.get("success") and element_search_result["elements"]:
            ...
            return {
                **state,
                "error_message": f"Element '{element_name}' not found in database",
                "current_step": "error"
            }
    except Exception as e:
        logger.error(f"Error in element search and setup: {e}")
        return {**state, "error_message": f"Element search failed: {str(e)}", "current_step": "error"}
...
def error_handler_agent(state: LineageState):
    ...
    error_result = {
        ...
        "suggestions": [
            ...
            "Verify MCP server connectivity and availability",
...
def should_continue(state: LineageState):
    ...
    elif current_step == "element_search_and_setup":
        return "element_search_and_setup"
...
class LineageOrchestrator:
    """Main orchestrator class for the intelligent lineage analysis workflow with MCP support."""
    ...
    def _build_workflow(self):
        """Build the LangGraph workflow with MCP-enabled agents."""
        workflow = StateGraph(LineageState)

        # Add all agent nodes - some are async now due to MCP calls
        workflow.add_node("coordinator", self._async_wrapper(lineage_coordinator_agent))
        workflow.add_node("contract_analysis", self._async_wrapper(contract_analysis_agent))
        workflow.add_node("element_analysis", self._async_wrapper(element_analysis_agent))
        workflow.add_node("element_search_and_setup", self._async_wrapper(element_search_and_setup_agent))
        ...
        workflow.add_conditional_edges("element_search_and_setup", should_continue)
    ...
    def _async_wrapper(self, async_func):
        """Wrapper to handle async functions in LangGraph."""
    ...
    async def execute_lineage_request(self, request: LineageRequest) -> Dict[str, Any]:
        """
        Execute a lineage analysis request with MCP server integration.
    ...
    async def resume_with_feedback(self, feedback: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Keep requires_human_approval as True so human_approval agent processes it
...
# --- MCP Server Health Check and Utility Functions ---
async def check_mcp_server_health() -> Dict[str, Any]:
    """Check if MCP server is accessible and healthy."""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{MCP_SERVER_BASE_URL}/health",
                                   timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    return {"healthy": True, "status": "MCP Server is accessible"}
                else:
                    return {"healthy": False, "status": f"MCP Server returned status {response.status}"}
    except Exception as e:
        return {"healthy": False, "status": f"Cannot reach MCP Server: {str(e)}"}


async def list_available_mcp_tools() -> Dict[str, Any]:
    """List all available tools from MCP server."""
    try:
        async with MCPClient() as client:
            url = f"{client.base_url}/mcp/tools"
            async with client.session.get(url) as response:
                if response.status == 200:
                    tools = await response.json()
                    return {"success": True, "tools": tools}
                else:
                    return {"success": False, "error": f"Failed to fetch tools: {response.status}"}
    except Exception as e:
        return {"success": False, "error": f"Error fetching tools: {str(e)}"}


# --- Example Usage ---
async def main():
    """Example usage of the MCP-enabled lineage orchestrator."""
    # Check MCP server health first
    health_check = await check_mcp_server_health()
    logger.info(f"MCP Server Health: {health_check}")

    if not health_check.get("healthy"):
        logger.error("MCP Server is not accessible. Please ensure it's running.")
        return

    # List available tools
    tools_check = await list_available_mcp_tools()
    logger.info(f"Available MCP Tools: {tools_check}")
    ...
    request = LineageRequest(
        query="Trace customer_name lineage",
        context="Need to understand data flow for customer_name field",
    ...
    if result.get("human_input_required"):
        logger.info(f"Human input required: {result['message']}")

        # Simulate human feedback (in real scenario, this would come from UI)
        feedback = {"selected_index": 0}  # Select first option

        # Resume with feedback
        final_result = await orchestrator.resume_with_feedback(feedback)
        logger.info(f"Final result: {json.dumps(final_result, indent=2, default=str)}")
    else:
        logger.info(f"Completed without human input: {json.dumps(result, indent=2, default=str)}")
...
# MCP Tool Functions
async def query_contract_by_name(contract_name: str) -> Dict[str, Any]:
    """Queries data contract table by contract name via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("query_contract_by_name", {"contract_name": contract_name})


async def query_pipelines_by_contract(contract_code: str) -> Dict[str, Any]:
    """Gets all ETL pipelines for a contract code via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("query_pipelines_by_contract", {"contract_code": contract_code})


async def query_pipeline_dependencies(query_codes: List[str]) -> Dict[str, Any]:
    """Gets downstream pipeline dependencies for a given list of query codes via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("query_pipeline_dependencies", {"query_codes": query_codes})


async def query_element_mappings_by_queries(query_codes: List[str]) -> Dict[str, Any]:
    """Gets element mappings for specific query codes via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("query_element_mappings_by_queries", {"query_codes": query_codes})


async def find_element_by_name(element_name: str) -> Dict[str, Any]:
    """Finds a data element by its name via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("find_element_by_name", {"element_name": element_name})


async def trace_element_connections(element_code: str, direction: str) -> Dict[str, Any]:
    """Traces connections for a data element via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("trace_element_connections", {
            "element_code": element_code,
            "direction": direction
        })


async def get_all_query_codes() -> Dict[str, Any]:
    """Dynamically fetch all available query codes from database via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("get_all_query_codes", {})


async def get_available_contracts() -> Dict[str, Any]:
    """Dynamically fetch all available contracts from database via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("get_available_contracts", {})


async def get_available_elements() -> Dict[str, Any]:
    """Dynamically fetch all available data elements from database via MCP server."""
    async with MCPClient() as client:
        return await client.call_tool("get_available_elements", {})
...
# Agent Functions (updated to use async MCP calls)
async def lineage_coordinator_agent(state: LineageState):
    ...
    # Dynamically fetch available data from database via MCP
    available_contracts = await get_available_contracts()
    available_elements = await get_available_elements()
...
async def contract_analysis_agent(state: LineageState):
    """LLM-powered contract analysis with intelligent insights via MCP."""
    logger.info("---EXECUTING: Intelligent Contract Analysis---")
    llm = get_llm()

    contract_name = state.get('contract_name', state.get('input_parameter'))

    # Gather contract data via MCP
    contract_details = await query_contract_by_name(contract_name)
    ...
    pipelines = await query_pipelines_by_contract(contract_code)
    query_codes = [p['query_code'] for p in pipelines.get("pipelines", [])]
    dependencies = await query_pipeline_dependencies(query_codes)
    mappings = await query_element_mappings_by_queries(query_codes)
...
async def element_analysis_agent(state: LineageState):
    """Intelligent element tracing with LLM-powered insights via MCP."""
    ...
    # Dynamically get all available query codes from database via MCP
    all_query_codes_result = await get_all_query_codes()
    ...
    # Get element mappings with LLM analysis using dynamic query codes via MCP
    all_mappings = await query_element_mappings_by_queries(query_codes)
